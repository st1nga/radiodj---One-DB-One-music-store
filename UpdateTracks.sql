...
-- Get current user, remove @<ip> from string
set @current_user = (select left(user(), locate("@", user()) - 1));
/*old! set @current_user = (select left(user(), 3));*/

/*Only update if this is called by the active studio*/
if @active_studio_user = @current_user then
  set @active_studio = 1;
...
INSERT INTO `history`(date_played, song_type, id_subcat, id_genre, duration, artist, original_artist, title, album, composer, `year`, track_no, disc_no, publisher, copyright, isrc, listeners, user, songid, active)
    SELECT NOW(), song_type, id_subcat, id_genre, duration, artist, original_artist, title, album, composer, `year`, track_no, disc_no, publisher, copyright, isrc, curListeners, @current_user, trackID, @active_studio FROM `songs` WHERE ID=trackID;
